...
<body>
	<div class="container">
		...
		<div>
			...
			<div class="content">
				<h3 style="margin-bottom: 0;">填写被申请方信息</h3><hr>
	  					...
	  					<c:choose>
    						<c:when test="${!empty(sessionScope.usertype) }">
	    						<c:if test="${sessionScope.usertype=='Consumer' }">
    								<form id="toApplyer_form_s">
		    							<ul>
								                <li><label class="label"><span class="need">*</span>修理商/销售商/制造商：</label>	
								                	<input type="text" value="${!empty(sessionScope.s)?sessionScope.s.name:'' }" name="s.name" datatype="*" nullmsg="请输入组织/法人的名称" class="inputxt" placeholder="组织/法人名称" /></li>
								                <li><label class="label">机构代码：</label>
								                	<input type="text" value="${!empty(sessionScope.s)?sessionScope.s.code:'' }" name="s.code" datatype="s8-9" ignore="ignore" errormsg="组织机构代码有误" class="inputxt" placeholder="输入组织机构代码" /></li>
								                <li><label class="label">负责人：</label>
								                	<input type="text" value="${!empty(sessionScope.s)?sessionScope.s.agent:'' }" name="s.agent" class="inputxt" placeholder="联系人（代理人）姓名"/></li>
	    							...
	    							<c:if test="${sessionScope.usertype=='Saler' }">
	    								<form id="toApplyer_form_c">
		    								<ul>
							                <li><label class="label"><span class="need">*</span> 姓名：</label>	
							                	<input type="text" value="${!empty(sessionScope.c)?sessionScope.c.name:'' }" name="c.name" class="inputxt" placeholder="申请者姓名"/></li>
							                <li><label class="label"> 身份证号：</label>
							                	<input type="text" value="${!empty(sessionScope.c)?sessionScope.c.idcard:'' }" name="c.idcard" class="inputxt" ignore="ignore" placeholder="输入身份证号码" /></li>
							                <li><label class="label">联系人：</label>
							                	<input type="text" value="${!empty(sessionScope.c)?sessionScope.c.agent:'' }" name="c.agent" class="inputxt" placeholder="联系人（代理人）姓名"/></li>
							                <li><label class="label"><span class="need">*</span> 联系电话：</label>
    							...
    							<div style="text-align: center; padding:40px 0;">
					  				<a href="applyer.jsp" class="nav-button"><img src="../../../js/easyui/themes/icons/back.png"  style="vertical-align: middle;">&nbsp; 上一步</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				...
				if(pass){//如果难通过
				    ...
				    $.post(url,params , function(r) {
						...
						if (r.ok) {
							...
							window.location=yp.bp()+"/page/user/apply/description.jsp"; 
						} else {
							layer.alert(r.msg, 8); 
						}
					}, 'json');
