...
public class GenericsUtils {
	/**
	 * 通过反射,获得指定类的父类的第index个泛型参数的实际类型。
	 * 
	 * @param clazz
	...
	public static Class getMethodGenericReturnType(Method method, int index) {
		...
		if (returnType instanceof ParameterizedType) {
			...
			if (index >= typeArguments.length || index < 0) { throw new RuntimeException("你输入的索引" + (index < 0 ? "不能小于0" : "超出了参数的总数")); }
	...
	public static List<Class> getMethodGenericParameterTypes(Method method, int index) {
		...
		if (index >= genericParameterTypes.length || index < 0) { throw new RuntimeException("你输入的索引" + (index < 0 ? "不能小于0" : "超出了参数的总数")); }
	...
	public static Class getFieldGenericType(Field field, int index) {
		...
		if (genericFieldType instanceof ParameterizedType) {
			...
			if (index >= fieldArgTypes.length || index < 0) { throw new RuntimeException("你输入的索引" + (index < 0 ? "不能小于0" : "超出了参数的总数")); }
